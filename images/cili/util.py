...
def pandas_dfs_from_asc(file_path):
    ...
    '''
    ORIGINALLY, the len(e_lines[ev_name]) > 0]) condition was excluding EBLINK,
    so I changed the cutoff to -2.  Not really sure what this means but it seems
    to have worked so I'm keeping it
    '''
    ...
    ev_dfs = dict([(ev_name,
                    ...
                    for ev_name in ev_names if len(e_lines[ev_name]) > -2])

    # TODO add omitting ASC_EV_IGNORE_COLUMNS[ev_name]

    return md.Samples.from_pd_obj(samp_df), md.Events.from_dict(ev_dfs)
