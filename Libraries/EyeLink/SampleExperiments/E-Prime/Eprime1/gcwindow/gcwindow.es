...
VersionInternal=4
VersionBuild=1
Logging("fgImageName").Log=-1
Logging("bgImageName").Log=-1
...
Levels=2
Levels(1).ValueString="1\t\tTrialProc\tSacrmeto.bmp\tSac_blur.bmp\t20000\t1\t"
Levels(2).ValueString="1\t\tTrialProc\tSac_blur.bmp\tSacrmeto.bmp\t20000\t2\t"
Name="TrialList"
TypeName="List"
Tag=""
Notes=""
...
VersionInternal=4
VersionBuild=1
_ItemList="Fixation;DriftCorrect;gcwindow"
_VersionPersist=1
FlowLines(0).Count=3
FlowLines(0).FlowItem(0).Name="Fixation"
FlowLines(0).FlowItem(1).Name="DriftCorrect"
FlowLines(0).FlowItem(2).Name="gcwindow"
LogData=1
Name="TrialProc"
TypeName="Procedure"
Tag=""
...
'Sub doGCWindow
'Input: 
'	fgName - Foreground image file name
'	bgName - Background image file name
...
'
'
Sub doGCWindow(fgName as String, bgName as String, DURATION as Integer, trialid as String, trial_word as String)
	Dim cnvs As Canvas
	Dim fg As Canvas
	Dim bg As Canvas
	...
	Set fg = Display.CreateCanvas		
	Set bg = Display.CreateCanvas

	Dim rct As Rect	


	cnvs.Clear
	cnvs.LoadImage bgName '"Sacrmeto.bmp"

	bg.Clear
	bg.LoadImage bgName '"Sacrmeto.bmp"

	fg.Clear
	fg.LoadImage fgName '"Sac_blur.bmp"
	...
	tracker.sendCommand "record_status_message 'GC " & trial_word & " (WINDOW)' "
   ...
   tracker.sendMessage "!V IMGLOAD FILL " & bgName
	...
	while elutil.currentTime() < stime + DURATION and button = 0 and tracker.isConnected() and not tracker.breakPressed()
		...
		if (Not s is Nothing) and lastSampleTime <> s.time and _
		s.gx(eye) >0 and s.gx(eye) <  (Display.XRes) and _
		s.gy(eye) >0 and s.gy(eye) <  (Display.YRes) then 
			lastSampleTime = s.time
			cnvs.Copy bg, rct, rct ' erase the last region
			
			x = s.gx(eye)
			y = s.gy(eye)
			rct.Left = x-wth/2
			rct.Top = y-hgt/2
			rct.Right = wth+rct.Left
			rct.Bottom = hgt+rct.Top
			...
			if rct.Bottom <0 then 
				rct.Bottom =0
			end if

			if rct.Left >Display.XRes then 
				rct.Left =Display.XRes
			end if
			if rct.Right >Display.XRes then 
				rct.Right =Display.XRes
			end if

			if rct.Top >Display.YRes then 
				rct.Top =Display.YRes
			end if

			if rct.Bottom >Display.YRes then 
				rct.Bottom =Display.YRes
			end if

			'Copy image to display canvas
			cnvs.Copy fg, rct, rct		
			tracker.sendMessage "SYNCTIME " & s.time
		End if
...
cancelout:
	...
	tracker.sendMessage "!V TRIAL_VAR foreground " & fgName
	tracker.sendMessage "!V TRIAL_VAR background " & fgName
