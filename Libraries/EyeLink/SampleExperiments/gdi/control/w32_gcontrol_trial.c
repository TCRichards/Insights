...
int gaze_control_trial(UINT32 time_limit)
{
  ...
  gbm = draw_grid_to_bitmap();   // Draw bitmap and EyeLink reference graphics
  ...
  sprintf(images_path,"%s/%s", session_path,"images"); 
  bitmap_save_and_backdrop(gbm, 0, 0, 0, 0, "grid.png", images_path, SV_NOREPLACE,
			      0, 0, BX_MAXCONTRAST|((eyelink_get_tracker_version(NULL)>=2)?0:BX_GRAYSCALE));
      ...
      if(i == FIXUPDATE)	  // only process FIXUPDATE events
	{
	   ...
	   if(evt.fe.eye == eye_used)	   // only process if it's from the desired eye?
	     {                             // get average position and duration of the update 
	       process_fixupdate(evt.fe.gavx, evt.fe.gavy,     // Process event 
