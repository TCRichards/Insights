...
int playback_trial(void)
{
    ...
    const unsigned int SAMPLEPLAYBACKOFFSET = 1;
	...
	while(1)   /* LOOP WHILE DATA AVAILABLE */
	{
		...
		else if(i==SAMPLE_TYPE)
		{
			...
			{
				sample_count ++;
                if (sample_count % SAMPLEPLAYBACKOFFSET == 0){
                    //msec_delay(SAMPLEPLAYBACKOFFSET*1000/sample_rate);  /* delay for real-time playback */
