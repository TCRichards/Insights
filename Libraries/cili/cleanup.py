...
def adjust_eyelink_recov_idxs(samples, events, z_thresh=.1, window=1000, kernel_size=100):
    ...
    """
    import numpy as np
    from cili.util import PUP_FIELDS
    ...
    reversed_dfs = dfs[::-1]
    # This may not be a real fix for rolling mean
    reversed_dfs_ravg = np.array(pd.Series(reversed_dfs).rolling(window=kernel_size, min_periods=1).mean())
